...
# Декоратор, що перехоплює порожній ввід
def skip_empty_input(func: Callable[[str], tuple[str, ...]]) -> Callable[[str], tuple[str, ...]]:
    """
    Якщо рядок вводу порожній/з пробілів — повертаємо ('',) як маркер відсутності команди.
    Це дозволяє не дублювати перевірки всередині parse_input.
    """
    @functools.wraps(func)
    def wrapper(user_input: str) -> tuple[str, ...]:
        if not user_input.strip():
            return ("",)  # важливо: тільки команда, без вкладених списків
